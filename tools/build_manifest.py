...
def build(root, repository, commit):
    ...
    if config.get('ready') is not True:
        ...
        return None
    if config['versions'] != {'minecraft': '1.21.10', 'neoforge': '21.10.64', 'java': 21}:
        raise ValueError('Versions attendues : Minecraft 1.21.10, NeoForge 21.10.64, Java 21')
    files, seen = [], set()
    for folder in ('mods', 'config', 'defaultconfigs', 'resourcepacks'):
        for path in sorted((root / folder).rglob('*')):
            ...
            files.append({'path': relative, 'url': f'https://raw.githubusercontent.com/{repository}/{commit}/' + quote(relative, safe='/'),
                          ...
                          'policy': 'seed' if folder in ('config', 'defaultconfigs') else 'enforced'})
